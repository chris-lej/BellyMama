...
const MapView = (props) => (
  <div>
    <div className="dropdown-background">
      <div className="d-flex justify-content-center dropdown-and-toggle">
        <SearchBar navBar />
        <ToggleView />
      </div>
    </div>
    ...
    <div className="map-container">
      {!!props.businessesWithoutAddress.length && (
        <NoAddressList
          businessesWithoutAddress={props.businessesWithoutAddress}
          wait={500}
        />
      )}
      <Map center={[props.centerLat, props.centerLong]} zoom={11}>
        <MapboxLayer accessToken={props.mapBoxToken} />
        {props.businessesWithAddress.map(
          ({ coordinates, name, phone, address, website }) => (
